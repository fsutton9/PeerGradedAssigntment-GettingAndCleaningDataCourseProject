...
setwd("C:Users/fsutton/Desktop/coursera/gettingandcleaningdata/week4/getdata_projectfiles_UCI HAR Data/ UCI HAR Dataset")
library(dplyr)
library(data.table)
...
Combining raw data sets into one set:

xDataset <- rbind(xTrain, xTest)
yDataset <- rbind(yTrain, yTest)
subjectDatast <- rbind(subjectTrain, subjectTest)
dim(xDataSet)
## [1] 10299 562
dim(yDataSet)
## [1] 10299 1
...
Create descriptive activity names to indicate the activies in the data set:
...
# To organize and combination of all datasets into a single set:

singleDataSet <- cbind(xDataSet_mean_std, yDataSet, subjectDataSet)

# We will define the descriptive names for all variables.
...
Now that we have a single data set, we will use the Tidy program to create a dataset with the average of each variable for each activity and each subject:
